...
- When identity evidence supplies a `failure_mode` and the current pressure activates it, the utterance must carry at least one audible trace of that mode: a restart, a word or number self-correction, a mid-sentence change of course, an over-long enumeration that loses its thread, or a repetition that comes back slightly wrong. A flawless finished speech under activated pressure is a voice-fidelity failure even when every sentence is idiomatic.
