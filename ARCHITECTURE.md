...
#### E. Human Operational Dialogue 与层级泄漏

人类对白只接收角色此刻可用的公开对象、已验证能力、当前社交动作和希望对方下一步执行的动作；规划字段、表单状态、完整条件—后果链和作者解释不得借角色之口进入表演。操作性交接按目击事件顺序组织，不按数据库字段顺序组织，并自然删除双方共享的规则与可见事实。一次发言在逼、问、拖、拒、求、骗或安抚落地后就停止，把后续解释留给对方追问。

生成只使用正向合同：具体人物／物件与动作、时间和空间指向、自然中文体貌与语气、允许改口／迟疑／局部回答，以及人类和非人类声音边界。固定禁句与词表不得进入生成 Prompt；否则模型只会绕写同一种句法。

`dialogue_naturalness` 是独立 Review 镜头，按机制执行层级泄漏、字段重建、目击顺序、共享上下文删减、朗读、删尾、角色互换和人机互换测试。人类对白若可原样交给协议机器人，或一次完整交付异常、机制、后果、操作范围、排除项和许可闭环，则该案例不得进入批次；逐案盲评不通过时必须重新生成。结构化 `对象 + 状态 + 结论` 只在非人角色声线合同明确支持时成立。
